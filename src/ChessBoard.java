    ...
    public boolean moveToPosition(int startLine, int startColumn, int endLine, int endColumn) {
        if (this.checkPos(startLine) && this.checkPos(startColumn)) {
            if (!this.nowPlayer.equals(board[startLine][startColumn].getColor())) {
                ...
                if (this.board[startLine][startColumn].getSymbol().equals("K") ||  // проверка позиции для рокировки
                        this.board[startLine][startColumn].getSymbol().equals("R")) {
                    this.board[startLine][startColumn].check = false;
                }

                this.board[endLine][endColumn] = this.board[startLine][startColumn]; // если фигура может двигаться, передвинули фигуру
                this.board[startLine][startColumn] = null; // установить null в предыдущую ячейку
        ...
        for (int i = 7; i > -1; i--) {
            ...
            for (int j = 0; j < 8; j++) {
                if (this.board[i][j] == null) {
                    ...
                    System.out.print(this.board[i][j].getSymbol() + this.board[i][j].getColor().substring(0, 1).toLowerCase() + "\t");
                }
            }
            System.out.println();
            System.out.println();
